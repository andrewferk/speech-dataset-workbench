...
| `ADR-00NN` | Part of it was superseded and the current decision lives in that ADR — read that one too. |
| `in place` | The ADR carries its own amendment notes, set against the text they correct. Read it and you have the current decision. |

A cell can hold both, where one part was superseded elsewhere and another was corrected at home.

## The records

| ADR | Decision | Amended by |
| --- | --- | --- |
| [0001](0001-identifier-scheme.md) | Ids are content-derived where identity is the bytes or the text (`recording_id`, `prompt_id`, `dataset_version`) and human-assigned where only the operator knows it (`speaker_id`, `session_id`). | ADR-0010 |
...
| [0004](0004-session-aware-splitting.md) | A whole Session lands in exactly one split, chosen by deterministic water-filling with a non-emptiness repair — so disjointness is session-level, not speaker-level. | in place |
...
| [0008](0008-testing-strategy-and-synthetic-fixtures.md) | Fixtures are synthesized in-repo rather than recorded; exact goldens pin the artifacts that are stable across machines, and build-twice-and-diff pins the bytes that are not. | in place |
| [0009](0009-seed-example-data.md) | `examples/` ships committed synthetic tones — 2 speakers, 4 sessions, ~12 recordings — shaped so the first run demonstrates splitting, the flag policy and speaker overlap, and labelled as tones rather than speech. | in place |
...
| [0012](0012-v0-1-acceptance-criteria.md) | v0.1 is done when CI is green, three checks pass (examples build, privacy allowlist, audit recipe) and a human has walked the example once — with no ADR-indexed checklist, which would be a second source of truth. | ADR-0014 |
